    ...
    public partial class frmPhongTro : Form
    {
       ...
       (
        ...
        private void picTimKiem_Click(object sender, EventArgs e)
        {
            ...
            String timKiem = txtTimKiem.Texts.Trim();
            int luaChon = 1;
            if (cmbTimKiem.Text == "Gần đúng")
                luaChon = 2;
            else if (cmbTimKiem.Text.Trim().ToUpper() == "ĐÃ THUÊ")
            {
                luaChon = 3;
                timKiem = "1";
            }
            else if(cmbTimKiem.Text.Trim().ToUpper() == "CHƯA THUÊ")
            {
                luaChon = 4;
                timKiem = "0";
            }
            dgvDSPT.DataSource = ptbll.timKiemPT(luaChon.ToString(), timKiem);
            if (timKiem.Length==0 &&(luaChon == 1 || luaChon == 2)) HienThiDSPT_DT();
