    ...
    public partial class frmPhieuChi : Form
    {
       ...
       (
        ...
        public void HienThiPC()
        {
            ...
            //}
            dgvDSPC.DataSource = pcBLL.hienThiDSPC();
        }
        //xoa
        ...
        private void dgvDSPC_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            if (e.RowIndex % 2 == 0) // Kiểm tra xem dòng có chỉ số chẵn hay không
                ...
                dgvDSPC.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.FromArgb(249, 173, 192);
            //255 182 193
        }
        public string strMaPC = "";
        private void dgvDSPC_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                strMaPC = selectedRow.Cells["MaPC"].Value.ToString().Trim();
                cmbMaPT.SelectedItem = selectedRow.Cells["MaPT"].Value.ToString().Trim();

                //ep kieu 
                DateTime NgayChi = DateTime.ParseExact(selectedRow.Cells["ngayChi"].Value.ToString().Trim(), "dd/MM/yyyy hh:mm:ss tt", CultureInfo.InvariantCulture);
        ...
        {
            if ( cmbMaPT.SelectedIndex == -1 || !double.TryParse(txtSoTien.Texts.Trim(), out _)) //txtMaPC.Texts.Trim().Length == 0 ||  txtLyDo.Texts.Trim().Length == 0
            {
        ...
        {
            bool ktMaPC = pcBLL.kiemTraMaPC(strMaPC.Trim());
          
            if (kiemTraPC())
            {
                PhieuChi phieuChi = new PhieuChi();
                phieuChi.MaPC = strMaPC;//txtMaPC.Texts.Trim();
                phieuChi.MaPT = cmbMaPT.SelectedItem.ToString();
        ...
        private void picTimKiem_Click(object sender, EventArgs e)
        {
            ...
            if(txtTimKiem.Texts.Trim().Length==0 )
                HienThiPC();
            else if(!pcBLL.kiemTraMaPC(strMaPC.Trim())) //tim kiem sai ko hien gì
                dgvDSPC.DataSource = pcBLL.timKiemPC(txtTimKiem.Texts.Trim());
            else
            {
                //dgvDSPC.Rows.Clear();//xóa các hàng trong grid view
                 //item = pcBLL.timKiemPC(txtTimKiem.Texts.Trim());
                //dgvDSPC.Rows.Add(item.MaPC, item.MaPT, item.NgayChi, item.SoTien, item.LyDo);
                dgvDSPC.DataSource = pcBLL.timKiemPC(txtTimKiem.Texts.Trim());
